...
| 10 | A frozen refusal probe retains **96–100%** across 8.5–4.5 bits on all three models, and falls to 63% at Phi-3.5-mini's largest shift, the 3.5-bit rung | Difference-in-means direction fitted on one prompt half, scored on a disjoint half, 200 synchronized splits | `review_reanalysis.py` → `probe` |
...
| "Quantization *significantly* increases the refusal class" | Withdrawn. Four graders from three other families re-scored the identical completions: three agree on the direction, none reaches significance, and the magnitude falls from 32-vs-11 to between 1-vs-2 and 12-vs-7. Pairwise agreement with the 7B judge is 64.9–89.3%, so grader disagreement alone moves more prompts than the effect | `compare_judges.py` |
