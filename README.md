...
> **The manuscript is not published from this repository.** `docs/paper/` is
> untracked: the paper, its figures, its tables and the consolidated measurement
> files are released deliberately rather than as a side effect of a commit. What
> is published is the code that produces all of them from `artifacts/runs`. The
> commands below regenerate the lot.
...
| Path | Purpose |
|---|---|
| `docs/paper/` | Manuscript, figures, tables and consolidated data — **not published here**, regenerated by the build scripts |
