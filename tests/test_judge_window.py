...
def test_the_window_is_part_of_the_cache_fingerprint() -> None:
    ...
    source = (REPO / "scripts" / "classify_completions_judge.py").read_text(
        encoding="utf-8")
    fingerprint = source.split("fingerprint = hashlib.sha256(")[1].split(
        ".hexdigest()")[0]
    assert "completion_chars" in fingerprint, (
        "the judge's completion window is not in the cache fingerprint, so "
        "re-judging at a different window would silently reuse stale verdicts")
